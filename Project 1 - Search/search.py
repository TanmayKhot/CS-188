...
def tinyMazeSearch(problem):
    ...
    def generic_search(problem, ds, add_to_ds_fn):
    closed = set()
    start = (problem.getStartState(), 0, [])  # (node, cost, path)
    add_to_ds_fn(ds, start, 0)

    while not ds.isEmpty():
        (node, cost, path) = ds.pop()
        ...
        if not node in closed:
            ...
            for child_node, child_action, child_cost in problem.getSuccessors(node):
                ...
                add_to_ds_fn(ds, new_state, new_cost)
...
def breadthFirstSearch(problem):
    ...
    ds = util.Queue()
    def add_to_ds_fn(ds, state, cost):
        ds.push(state)

    return generic_search(problem, ds, add_to_ds_fn)
